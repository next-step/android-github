...
internal class GithubRepositoryImplTest {
    ...
    fun 저장소를_불러오는_메소드를_호출했을_때_반환되는_값을_확인한다() = runTest {
        //given
        val response = listOf<GithubRepoResponse>(GithubRepoResponse(-1, "test", "test description"))
        coEvery { githubApiService.repositories() } returns response
        val expected = response.map(GithubRepoResponse::toDomain)
        ...
        //then
        coVerify { githubApiService.repositories() }
        assertThat(actual.list).isEqualTo(expected)
    }
}